...
void DeformCylinder()
{
	//Precalculate per-bone transformations according to the book at chapter 11.6, page 189
	mat4 Mbm[kMaxBones];
	mat4 Mmb[kMaxBones];
	int bone;
	for(bone = kMaxBones-1; bone > -1; --bone)
	{
		...
		if(bone != 0)
		{
			restTranslation = VectorSub(g_bonesRes[bone].pos, g_bonesRes[bone-1].pos);
		} else {
			restTranslation = g_bonesRes[bone].pos;
		}
		mat4 tRest = T(restTranslation.x, restTranslation.y, restTranslation.z);
		mat4 rRest = g_bones[bone].rot;
		mat4 rAnim = g_bonesRes[bone].rot;
		mat4 modifiedTransformation = Mult(tRest, Mult(rRest, rAnim));
		...
		//Fill transformationlist
		int i;
		Mmb[bone] = inverseBaseTransformation;
		Mbm[bone] = modifiedTransformation;
		for(i = bone; i < kMaxBones; ++i)
		{
			//Mmb[i] = Mult(Mmb[i], Mmb[bone]); //Not needed since we have the model coordinates and can invert them straight away. This only needed when all we have is relative coordinates.
			Mbm[i] = Mult(Mbm[bone], Mbm[i]);
		}
	}

	mat4 transformations[10];
	for(bone = 0; bone < kMaxBones; ++bone)
	{
		transformations[bone] = Mult(Mbm[bone], Mmb[bone]);
	}

	
	glUniformMatrix4fv(glGetUniformLocation(g_shader, "matrices"), kMaxBones, 0, &transformations[0].m[0]);
}
