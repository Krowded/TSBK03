...
#define epsilon 0 //Elasticityconstant for all balls
...
void updateWorld()
{
	...
	for (i = 0; i < kNumBalls; i++)
        for (j = i+1; j < kNumBalls; j++)
        {
            ...
            float distanceBetweenBalls = Norm(collisionVector) - 2*kBallSize;
            ...
            //Force them apart
            float overlap = distanceBetweenBalls;
            ball[i].X = VectorAdd(ball[i].X, ScalarMult(normal, -overlap/2));
            ball[j].X = VectorAdd(ball[j].X, ScalarMult(normal, overlap/2));

            //Change momentum
            vec3 velocityBalli = ScalarMult(ball[i].P, 1.0/ball[i].mass);
            vec3 velocityBallj = ScalarMult(ball[j].P, 1.0/ball[j].mass);
            vec3 deltaVBefore = VectorSub(velocityBalli, velocityBallj);
	...
	for (i = 0; i < kNumBalls; i++)
	{
        ...
        float velocity = Norm(ScalarMult(ball[i].P, 1.0/ball[i].mass));
